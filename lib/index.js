...
// (port 3080 + GET /modlens/paste + GET /) and writes the verdict into
// D:\dsh-web\轻量事务\.dsh-restart-live.json plus E:\pi-windows\dsh-web-restart-status.txt.
import { readFile } from 'node:fs/promises'

const LIVE = 'D:\\dsh-web\\轻量事务\\.dsh-restart-live.json'
...
export function apply(ctx) {
  ...
  disposers.push(ctx.effect(() => ctx.tools.register({
    name: 'dsh_restart',
    description: '触发 DSH Web 环境重启（固定 10 秒倒计时模式）。调用后：计划任务 dsh-web-restart-20s 立即启动，10 秒后杀掉 127.0.0.1:3080 的旧 DSH 宿主进程并用同一方式拉起新实例，然后自动自检（3080 端口监听 + GET /modlens/paste + GET /），结果写入 E:\\pi-windows\\dsh-web-restart-status.txt 与 D:\\dsh-web\\轻量事务\\.dsh-restart-live.json。浏览器页面会在倒计时期间整体变灰，重启完成后自动重连并刷新。注意：本回合的宿主进程将在倒计时结束后被杀，所以调用本工具后必须立即结束回合、只输出一句简短的中文通知，不要再调用任何其他工具。',
    parameters: {
      type: 'object',
      properties: {},
    },
