...
function apply(ctx) {
  ...
  function findSettingsBtn() {
    for (const b of document.querySelectorAll("button")) {
      ...
      if (t === "设置" || a === "设置" || a.indexOf("设置") >= 0 || /settings/i.test(a)) return b;
    }
    // 兜底：侧边栏底部区域（footArea）里宽按钮（展开态 42 高）
    const foot = document.querySelector(".hHd-Xa_footArea");
    if (foot) {
      for (const b of foot.querySelectorAll("button")) {
        const r = b.getBoundingClientRect();
        if (r.width >= 60) return b;
      }
    }
    return null;
  }
  ...
  // 这里按设置按钮 rect 实时对齐其右侧，React 重渲染不会触碰该按钮。
  // 按设置按钮 rect 实时对齐展开态按钮：右缘距设置按钮右缘 8px、垂直居中
  function repositionWide(settingsBtn) {
    if (!wideBtn || !settingsBtn) return;
    const r = settingsBtn.getBoundingClientRect();
    const w = wideBtn.offsetWidth || 61;
    wideBtn.style.left = Math.round(r.right - 8 - w) + "px";
    wideBtn.style.top = Math.round(r.top + (r.height - 26) / 2) + "px";
  }

  function ensureWide(settingsBtn) {
    if (!wideBtn) {
      ...
      btn.style.cssText =
        ...
        "border:1px solid rgba(120,130,150,0.45);background:rgba(255,255,255,0.85);" +
        "color:#444b58;font-size:12px;line-height:24px;cursor:pointer;z-index:60;" +
  ...
  function ensureRail(root) {
    if (!railBtn) {
      ...
      btn.style.cssText =
        ...
        "border:none;background:transparent;color:rgb(15,17,21);cursor:pointer;z-index:60;" +
  ...
  function layout() {
    ...
    if (collapsed) {
      ...
      removeRail();
      if (settingsBtn) ensureWide(settingsBtn);
    }
  }
  ...
  try { console.info("[dsh-restart] client v11 mounted (hardened: fetch-timeout, cached-layout, dispose-clean, singleton, observers, auto-reload, toast)"); } catch (e) { /* 忽略 */ }
